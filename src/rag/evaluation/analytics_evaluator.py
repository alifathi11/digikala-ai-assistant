...
class ManagerAnalyticsEvaluator:
    ...
    @staticmethod
    def _judge_context_from_pipeline_context(
        pipeline_context,
    ):
        return {
            "scope": (
    ...
    def rejudge_case_ids(
        self,
        checkpoint_path,
        case_ids,
    ):
        """
        Re-run only the qualitative judge for successful checkpoint cases.

        The deterministic analytics context is rebuilt locally, so the
        original Manager Answer LLM call is not repeated.
        """
        ...
        for case_id in (
            ...
            if case_id not in (
                ...
                raise ValueError(
                    f"Unknown analytics case_id: {case_id}"
                )
            ...
            if case_id not in (
                ...
                raise ValueError(
                    f"No successful checkpoint result for {case_id}"
                )
            ...
            compact[
                ...
                self._judge_context_from_pipeline_context(
                    rebuilt_context
                )
            )
            ...
            print(
                f"{case_id}: judge-only rerun "
                f"score={row['overall_judge_score']:.2f}"
            )
